  ...
  return (
    <Panel title="System monitor" className="mt-5 mb-5">
      ...
      {metrics && <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6">
        ...
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900 dark:bg-amber-950/40"><div className="mb-3 flex items-center justify-between text-amber-700 dark:text-amber-300"><span className="text-sm font-medium">Disk health</span><HeartPulse size={18} /></div><p className="text-lg font-bold">{metrics.temperature.celsius === null ? "N/A" : `${metrics.temperature.celsius.toFixed(1)} °C`}</p><p className="mt-1 truncate text-xs text-slate-500 dark:text-slate-400" title={metrics.diskHealth.message}>{metrics.diskHealth.status === "healthy" ? "Disk SMART healthy" : metrics.diskHealth.status === "warning" ? "Disk SMART warning" : "Disk health unavailable"}</p></div>
      </div>}
