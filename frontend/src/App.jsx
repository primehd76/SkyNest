...
function Drive({ token, user, openAdmin }) {
  ...
  async function selectFolder(folder, nextTrail = [folder]) { setActive(folder); setMenu(null); setFolderMenu(null); setTrail(nextTrail); try { await refreshFiles(folder); } catch (e) { showError(e); } }
  async function createSubfolder() {
    if (!active) return;
    const folderName = prompt("Folder name"); if (!folderName) return;
    const quotaGb = prompt("Quota in GB", "1"); if (quotaGb === null) return;
    const quotaLimitBytes = Math.round(Number(quotaGb) * 1024 ** 3);
    if (!Number.isSafeInteger(quotaLimitBytes) || quotaLimitBytes < 0) return setMessage("Quota must be a non-negative number of GB.");
    try { await api.post(`/folders/${active.id}/subfolders`, { folderName, quotaLimitBytes }, { headers }); await refreshFiles(active); setMessage("Folder created."); }
    catch (e) { showError(e); }
  }
